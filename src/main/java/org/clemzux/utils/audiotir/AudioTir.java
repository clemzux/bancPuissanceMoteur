...
public class AudioTir {
    ...
    private List<Float> spectrumVariations;
    ...
    private void calculateNewtonMetersCurve() {
        ...
        int nbCasesBy10mSec = (int) (spectrumLength / duration);
        nbCasesBy10mSec /= 10;
        System.out.println(spectrumLength);
        System.out.println(nbCasesBy10mSec);

        List<Float> spectrumVariations = new ArrayList<>();
        ...
        // cette boucle sert a calculer le nombre d'explosions tout les 10emes de secondes
...
//        System.out.println("nombre cases : " + spectrumVariations.size());
//        for (int round : spectrumVariations)
//            System.out.println(round);

        index = 0;

        // on applique le coeficient aux variations trouvees
        while (index < spectrumVariations.size()) {

            spectrumVariations.set(index, spectrumVariations.get(index) * inertiaMoment);
//            System.out.println(spectrumVariations.get(index) * inertiaMoment);
            index++;
        }

        this.spectrumVariations = spectrumVariations;
    }
    ...
    public List<Float> getSpectrumVariations() { return spectrumVariations; }
}
